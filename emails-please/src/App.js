...
function App() {
    ...
    const handleFieldClick = (field) => {
        ...
        if (
            currentEmail.ErrorType === selectedError && 
            currentEmail.ErrorField.toLowerCase() === field.toLowerCase() 
        ) {
            setMatchStatus("match found");
            setAccuracyScore((prev) => Math.min(prev + 10, 100));
        } else {
            setMatchStatus("no match found");
            setAccuracyScore((prev) => Math.max(prev - 20, 0));
        }
    };
    ...
    return (
        <div className="app-container">
            ...
            <Sidebar
                criteria={["Grammatical Error", "Spam", "Phishing"]}
